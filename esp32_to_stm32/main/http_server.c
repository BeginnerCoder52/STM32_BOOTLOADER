...
static esp_err_t send_command(const char* command) {
    ...
    int bytes_written = uart_write_bytes(UART_PORT_NUM, command, len);
...
static esp_err_t wait_for_response(const char* expected_response, uint32_t timeout_ms) {
    ...
    while ((xTaskGetTickCount() - start_time) < timeout_ticks) {
        ...
        if (bytes_read > 0) {
            if (buffer_pos < (MAX_RESPONSE_SIZE - 1)) {
                ...
                if (strstr(response_buffer, expected_response) != NULL) {
                    ESP_LOGI(TAG, "Received expected response: %s", expected_response);
                    return ESP_OK;
                }

                // Reset buffer if we get \n (end of line)
                if (byte == '\n') {
                    ESP_LOGW(TAG, "Received unexpected response: %s", response_buffer);
                    memset(response_buffer, 0, sizeof(response_buffer));
                    buffer_pos = 0;
                }
            } else {
    ...
    ESP_LOGE(TAG, "Timeout waiting for response: %s", expected_response);
...
static esp_err_t http_server_upload_handler(httpd_req_t *req) {
    ...
    if (raw_file_size > 50) {  // Need minimum size to have boundary
...
static esp_err_t http_server_download_handler(httpd_req_t *req) {
    ...
    // Step 1: Send FW_REQUEST command
    ESP_LOGI(TAG, "Step 1: Sending FW_REQUEST");
    if (send_command(CMD_FW_REQUEST) != ESP_OK) {
        free(firmware_data);
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to send FW_REQUEST");
        return ESP_FAIL;
    }

    // Step 2: Wait for FW_READY response
    ESP_LOGI(TAG, "Step 2: Waiting for FW_READY");
    if (wait_for_response(RESP_FW_READY, PROTOCOL_TIMEOUT_MS) != ESP_OK) {
        free(firmware_data);
    ...
    char length_command[64];
    snprintf(length_command, sizeof(length_command), "%s%ld\r\n", CMD_FW_LENGTH, file_size);
    ESP_LOGI(TAG, "Step 3: Sending FW_LENGTH: %ld bytes", file_size);
    if (send_command(length_command) != ESP_OK) {
        free(firmware_data);
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to send FW_LENGTH");
        return ESP_FAIL;
    }

    // Step 4: Wait for FW_OK response
    ESP_LOGI(TAG, "Step 4: Waiting for FW_OK");
    if (wait_for_response(RESP_FW_OK, PROTOCOL_TIMEOUT_MS) != ESP_OK) {
        free(firmware_data);
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "STM32 rejected firmware length");
        return ESP_FAIL;
    }
    ...
    while (offset < file_size) {
        ...
        // Send FW_DATA command with chunk
        char data_command[16];
        snprintf(data_command, sizeof(data_command), "%s", CMD_FW_DATA);

        // Send command header
        if (send_command(data_command) != ESP_OK) {
            ESP_LOGE(TAG, "Failed to send FW_DATA command");
            free(firmware_data);
            httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to send FW_DATA command");
            return ESP_FAIL;
        }
    ...
    // Wait for either CHECKSUM_OK or CHECKSUM_ERR
    char response_buffer[MAX_RESPONSE_SIZE];
    memset(response_buffer, 0, sizeof(response_buffer));
    ...
    while ((xTaskGetTickCount() - start_time) < timeout_ticks && !response_received) {
        ...
        if (bytes_read > 0) {
            if (buffer_pos < (MAX_RESPONSE_SIZE - 1)) {
                ...
                if (strstr(response_buffer, RESP_CHECKSUM_OK) != NULL) {
                ...
                if (strstr(response_buffer, RESP_CHECKSUM_ERR) != NULL) {
